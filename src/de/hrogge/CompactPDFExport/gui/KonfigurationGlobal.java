...
public class KonfigurationGlobal implements IKonfigurationsSeite {
	private static final String XML_NODENAME = "property";
	private static final String XML_LISTNAME = "global";
	...
	public void appendToXml(Document d, Element parent) {
		...
		xmlList = d.createElement(XML_LISTNAME);
		parent.appendChild(xmlList);

		for (String key : textMap.keySet()) {
			JTextField tf = textMap.get(key);

			child = d.createElement(XML_NODENAME);
			child.setAttribute("key", key);
			child.setAttribute("value", tf.getText());
			xmlList.appendChild(child);
		}
		...
		for (String key : optionenMap.keySet()) {
			...
			child = d.createElement(XML_NODENAME);
			child.setAttribute("key", key);
			child.setAttribute("value", Boolean.toString(tb.isSelected()));
			xmlList.appendChild(child);
		}
	}

	public boolean getOptionsDaten(String key) {
		if (!optionenMap.containsKey(key)) {
			return false;
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.hrogge.CompactPDFExport.gui.IKonfiguration#getPanel()
	 */
	@Override
	...
	public void readFromXML(Element root) {
		NodeList xmlList = root.getElementsByTagName(XML_LISTNAME);

		if (xmlList.getLength() == 1 && xmlList.item(0) instanceof Element) {
			Element parent = (Element) xmlList.item(0);
			/* initialize defaults */
			for (String key : textMap.keySet()) {
				textMap.get(key).setText(textStandardMap.get(key));
			}
			for (String key : optionenMap.keySet()) {
				optionenMap.get(key).setSelected(optionenStandardMap.get(key));
			}

			/* read xml */
			NodeList list = parent.getElementsByTagName(XML_NODENAME);
			for (int i = 0; i < list.getLength(); i++) {
				...
				JToggleButton tb = optionenMap.get(key);
				if (tb != null) {
					tb.setSelected(Boolean.parseBoolean(value));
				}
			}
		}
	}
	...
	private void erzeugeFrontPanel() {
		...
		fKaufbareEigenschaften.setAlignmentX(Component.LEFT_ALIGNMENT);
		...
		fMehrSF.setAlignmentX(Component.LEFT_ALIGNMENT);
	...
	private void erzeugeTalentPanel() {
		...
		tiLeereSpalten.setAlignmentX(Component.LEFT_ALIGNMENT);
		...
		tBasisTalente.setAlignmentX(Component.LEFT_ALIGNMENT);
	...
	private void erzeugeZauberPanel() {
		...
		ziRepraesentation.setAlignmentX(Component.LEFT_ALIGNMENT);
		zImmerPanel.add(ziRepraesentation);
		optionenMap.put(ZAUBER_IMMER_REPRAESENTATION, ziRepraesentation);

		zNotizenPanel = new JPanel();
		zNotizenPanel.setLayout(new BoxLayout(zNotizenPanel, BoxLayout.PAGE_AXIS));
		...
		zHauszauberOben.setAlignmentX(Component.LEFT_ALIGNMENT);
		...
		zSeitenzahlen.setAlignmentX(Component.LEFT_ALIGNMENT);
