...
public class SFSeite extends PDFSeite {
	public SFSeite(PDDocument d, float marginX, float marginY,
	...
	public boolean erzeugeSeite(String[] guteEigenschaften,
		...
		for (int i=1; i<alleSF.size(); i++) {
			if (alleSF.get(i-1).getKategorie() != alleSF.get(i).getKategorie()) {
				alleSF.add(i, null);
				i++;
			}
		}
		...
		filter(alleSF);
		PDFSonderfertigkeiten.zeichneTabelle(this, 0, 2, 20, cellCountY, "Sonderfertigkeiten 1", alleSF);

		filter(alleSF);
		PDFSonderfertigkeiten.zeichneTabelle(this, 21, 2, 42, cellCountY, "Sonderfertigkeiten 2", alleSF);
		
		filter(alleSF);
		PDFSonderfertigkeiten.zeichneTabelle(this, 43, 2, 63, cellCountY, "Sonderfertigkeiten 3", alleSF);
